    ...
    def to_unweighted(self):
        if hasattr(self, 'edge_weight'):
            if self.edge_weight.size(0) != self.edge_index.size(1):
                print(f"警告: 原始数据中边索引和边权重尺寸不匹配")
                print(f"边索引尺寸: {self.edge_index.size()}, 边权重尺寸: {self.edge_weight.size()}")
                
                ratio = self.edge_weight.size(0) / self.edge_index.size(1)
                print(f"权重与索引比例: {ratio:.2f}")
                
                if abs(ratio - 2.0) < 0.01:
                    print("检测到可能的无向图结构 (边权重是边索引的2倍)")
                    edge_index_orig = self.edge_index
                    edge_index_rev = torch.stack([edge_index_orig[1], edge_index_orig[0]], dim=0)
                    
                    self.edge_index = torch.cat([edge_index_orig, edge_index_rev], dim=1)
                    
                    self.edge_weight = self.edge_weight.sign()
                
                elif abs(ratio - 0.5) < 0.01:
                    print("检测到可能的无向图结构 (边索引是边权重的2倍)")
                    self.edge_weight = self.edge_weight.sign().repeat(2)[:self.edge_index.size(1)]
                
                else:
                    print(f"检测到非标准边比例: {ratio:.2f}")
                    num_nodes = self.num_nodes
                    row_indices = self.edge_index[0].cpu().numpy()
                    col_indices = self.edge_index[1].cpu().numpy()
                    
                    if len(row_indices) <= self.edge_weight.size(0):
                        values = self.edge_weight.sign().cpu().numpy()[:len(row_indices)]
                    else:
                        repeat_count = (len(row_indices) + self.edge_weight.size(0) - 1) // self.edge_weight.size(0)
                        values = np.tile(self.edge_weight.sign().cpu().numpy(), repeat_count)[:len(row_indices)]
                    
                    self.A = sp.coo_matrix((values, (row_indices, col_indices)), shape=(num_nodes, num_nodes))
                    self.edge_weight = torch.FloatTensor(self.A.data)
                    self.edge_index = torch.LongTensor(np.array(self.A.nonzero()))
            else:
                self.edge_weight = self.edge_weight.sign()
